...
# material_taxa_import  <- read.csv(here('translation','input','material_taxa.csv'))
# c14raw=left_join(x=c14raw,y=material_taxa_import,by="MaterialType")
c14raw$Taxa = NA # These needs to be removed once the above is complete
```
...
write.csv(c14kudocheck,file=here('translation','output','c14db_toCheck.csv'),row.names=FALSE)
```
...
c14db  <- select(c14db,LabCode,PrefectureNameEn,Region,SiteNameJp=SiteName,SiteNameEn=Romanised,SiteType=SiteType,Latitude,Longitude,CRA,CRAError,UnroundedCRAError,UnroundedCRA,Delta13C,Delta13CError,Delta13CIRMS,Material,MaterialDetails=Material_Details,Taxa=Taxa,NabunkenURL=nabunkenURL,Reference=ReferenceCombined)
write.csv(c14db,file=here('translation','c14db_0.1.0.csv'))
saveRDS(c14db,file=here('translation','c14db_0.1.0.Rds'))
```
