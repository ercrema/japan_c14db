...
c14raw=read.csv(here('translation','input','c14_raw_v220303.csv'),na.strings = '',skip=2)
```
...
for (i in 1:length(duplicates))
{
  ...
  if(labcodename=="不明") {
	  c14raw$labcode_issue[index.labcodename] = 'Missing or Unknown'
  ...
  if (tmp.number.cases>1) # if some variations in info
	  {
		  c14raw$labcode_issue[index.labcodename] = ''
		  if(length(unique(tmp$PrefectureCode))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different Prefecture',sep=';')}
		  if(length(unique(tmp$SiteName))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different SiteName',sep=';')}
		  if(length(unique(tmp$MaterialCode1))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different MaterialCode1',sep=';')}
		  if(length(unique(tmp$MaterialCode2))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different MaterialCode2',sep=';')}
		  if(length(unique(tmp$Method))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different Method',sep=';')}
		  if(length(unique(tmp$CRA))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different CRA',sep=';')}
		  if(length(unique(tmp$CRAError))>1){c14raw$labcode_issue[index.labcodename]=paste(c14raw$labcode_issue[index.labcodename],'Same labcode, different CRAError',sep=';')}
...
pref.lookup = read.csv(here('translation','input','prefectures_translations.csv'),stringsAsFactors=FALSE)
...
```{r}
source(here('translation','utility','checkPrefecture.R'))
jwgs84 = readOGR(dsn=here('translation','input','gis'),layer='japan_wgs84')
jjgd2000 = readOGR(dsn=here('translation','input','gis'),layer='japan_jgd2000')
...
c14raw[ii,c('Prefecture','Latitude','Longitude')]
c14raw$coord.prefecture.check[which(c14raw$SiteLocation%in%c(manual.prefecture.check$SiteLocation[16]))[5:6]]='Coordinate in Gunma but Prefecture recorded as Nagano'
```
...
**NOTE**: A key decision to be made is whether site name should be reported verbatim (including e.g. excavation number) or should english name just extract the site name itself. The best option is to use the name as it is but point out in the JOAD paper that this should not be used as an analytical unit.  

```{r}
library(Nippon)
source(here('translation','utility','siteNameTranscript.R'))
...
sitenames=read.csv(here('translation','input','sitenamesTranscript.csv'),stringsAsFactors = FALSE)
...
if (!all(unique(siteList)%in%sitenames$SiteName))
{
  ...
  sitenames=rbind.data.frame(sitenames,data.frame(SiteName=additional_sitenames$SiteName,
                                                      ...
                                                      ToCheck=TRUE,
                                                      Alternative=NA))
  write.csv(sitenames,here('translation','output','sitenamesTranscript_toprocess.csv'),row.names = FALSE)
}
...
c14raw$SiteNameIssues = NA
ii = which(c14raw$Alternative!='')
c14raw$SiteNameIssues[ii] = paste0('Site Name should be',c14raw$Alternative[ii])
...
```{r}
# Read Lookup tables
matcode1 = read.csv(here('translation','input','materialGeneralCode.csv'))
matcode2 = read.csv(here('translation','input','materialDetailsCode.csv'))
...
# And export
write.csv(material_taxa,file=here('translation','output','material_taxa_todo.csv'),row.names=FALSE)
```
...
The final database should:
  ...
  * exclude all instances of other major inconsistencies.
...
index.above.threshold = which(absDiff>threshold)
c14raw$toKeep[index.above.threshold] = FALSE
c14raw$c14_issues[index.above.threshold] = "difference between unrounded and CRA >",threshold) 
```
