...
c14db  <- select(c14db,originalRow,LabCode,PrefectureNameEn,Region,SiteNameJp=SiteName,SiteNameEn=Romanised,SiteType=SiteType,Latitude,Longitude,CRA,CRAError,UnroundedCRAError,UnroundedCRA,Delta13C,Delta13CError,Delta13CIRMS,Material,MaterialDetails=Material_Details,Taxa=Taxa,NabunkenURL=nabunkenURL,Reference=ReferenceCombined)
write.csv(c14db,file=here('c14db_0.2.1.csv'))
saveRDS(c14db,file=here('c14db_0.2.1.Rds'))
```
