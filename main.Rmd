...
c14raw=read_xlsx( "./raw/c14_raw_v200130.xlsx", skip=1) %>% as.data.frame(stringsAsFactor=FALSE)
```
...
write.csv(c14raw.labcodeissue,"./reports/missingLabCodes.csv",row.names=FALSE) 
```
...
write.csv(duplicatedLabcodes,"./reports/duplicateLabCodes.csv",row.names=FALSE) 
```
...
gmap.coord=read.csv(file="./output/gmap.coord.csv",stringsAsFactors = FALSE)
...
gmap.coord.extra=extractCoordinates(missing.Addresses)
...
write.csv(c14raw[missing.coord.index,],"./reports/nocoordinates_and_noreliableaddress.csv",row.names=FALSE) 
```
...
Manual check. `r tmp$Sitename[3]` is in Kagoshima and `r tmp$Sitename[4]` is in Sapporo, although both are recorded as in Hokkaido. 

```{r}
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[1:2]))]=TRUE
```

These two cases should also have retain set to FALSE:

```{r}
c14raw$retain[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[3:4]))]=FALSE
```
...
The database does not provide unique identifier for sites. Thus several sites with different names might refer to the same site, but equally because coordinates were often derived from addresses, multiple sites with different names might have the same spatial coordinate as well. In remote cases the same site might have also different coordinates in different publications. 
...
write.csv(d,"./reports/samesite_differentcoordinates.csv",row.names=FALSE)
```
...
<!-- stored everything till this point in temp_c14raw.RData -->
...
sitenames=read.csv("./lookup/sitenamesTranscript.csv",stringsAsFactors = FALSE,na.strings = '')
...
if (any(!is.na(c14raw$Alternative)))
{
 ...
 write.csv(fix.names,file='./reports/sitenames_issues.csv')
}
...
## Store Output
```{r}
write.csv(c14raw,file='japanc14db_v01(190801).csv')
c14db=c14raw
save(c14db,file='japanc14db_v01(190801).RData')
```
