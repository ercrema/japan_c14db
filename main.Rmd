...
We first read the most up-to-date version of the raw data (currently `c14_raw_v210715.csv` corresponding to the original file `★＜公開中＞年代測定データベース学術・関東･東北・北陸・中部・九州・北海道・四国・近畿20210715.xlsx`, without the first row):

```{r}
options(warn=-1) #suppress warning
# c14raw=read_xlsx(here('raw','c14_raw_v210715.xlsx'), skip=1) |> as.data.frame(stringsAsFactor=FALSE)
c14raw=read.csv(here('raw','c14_raw_v210715.csv'),na.strings = '',skip=2)
...
### FIX CorrectedC14Age Field
The field has non-numeric entries so these needs to be converted into NAs.

```{r}
c14raw$UnroundedCRA[grep('[^0-9.-]',c14raw$UnroundedCRA)]=NA
...
which does not seem to be the case. Indeed the majority (`r length(which(!c14raw.rekihakuProj$LabCode%in%c14raw.NonRekihakuProj$LabCode))` vs `r length(which(c14raw.rekihakuProj$LabCode%in%c14raw.NonRekihakuProj$LabCode))`) seems to actually not be present in the rest of the database.
...
{ 
  ...
  tmp = c14raw[tmp.index,c("PrefectureCode","SiteName","SiteLocation","SamplingLocation","MaterialType","MaterialCode1","MaterialCode2","PeriodCode","LabCode","Method","CRA","CRAError","UnroundedCRA","UnroundedCRAError","Delta13C","Delta13CError")] 
...
# 徳島       大谷尻遺跡 (coodinate in Aichi)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[9]))]=FALSE
# 奈良       観音寺本馬遺跡 (coodinate in Yamagata)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[10]))]=FALSE
# 和歌山       徳蔵地区遺跡 (coodinate in Wakayama)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[11]))]=TRUE
# 和歌山       太田・黒田遺跡 (coodinate in Wakayama)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[12]))]=TRUE
# 和歌山       丁ノ町・妙寺遺跡第2次 (coodinate in Wakayama)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[13]))]=TRUE
# 和歌山       井辺遺跡 (coodinate in Wakayama)
c14raw$coord.check[which(c14raw$SiteLocation%in%c(tmp$SiteLocation[14]))]=TRUE
```
...
```{r}
threshold = 50
absDiff = abs(c14raw$CRA-c14raw$UnroundedCRA)
ii=rev(which(sort(absDiff,TRUE)>threshold))[1] #discrepancies over threshold years
largeDiscrep = c14raw[order(absDiff,decreasing = TRUE)[1:ii],]

write.csv(largeDiscrep,here('reports','c14age_discrpancies.csv'),row.names=FALSE)

c14raw$retain[which(absDiff>threshold)] = FALSE #set large discrepancies dates as retain=FALSE
```
...
## Material Details
```{r}
# General Notes on File Names
# material_details_before.csv... was the first original export
# material_details_after... after CJS's translation
# material_details_todo ... updated version with new entries, must be saved as 'material_details_after' after translations with fields: "MaterialTypeUniqueID"  "Material_Classification", "Material_Details_JPN" ,"inconsistent" ,"Materials_Details_EN","Materials_Detail_Identification_EN","Materials_Detail_Other_EN", and "Materials_Detail_Notes_EN"    

# Initial Cleaning to facilitate merging
...
# First time around: 
# c14raw$MaterialTypeUniqueID = as.numeric(factor(c14raw$MaterialType)) #Add Unique Identifier
# material_details = unique(data.frame(MaterialTypeUniqueID=c14raw$MaterialTypeUniqueID,Material_Classification=c14raw$Material_Details,Material_Details_JPN=c14raw$MaterialType,Material_Details_EN=NA)) #generate lookup table
...
# # sort to facilitate
# material_details=material_details[order( material_details$Material_Classification, material_details$Material_Details_JPN ),]
# write.csv(material_details,file="./lookup/material_details_before.csv",row.names=F) #export to lookup table

# Second Time Around
#material_details_after=read.csv(here('lookup','material_details_after.csv')) #read file
#material_details_after=read.csv(here('lookup','material_details_from_chris_2021_07_13.csv')) #read file
material_details_after=read.csv(here('lookup','Material_details_2021_08_17_CJS.csv')) #read file

if (any(!unique(as.character(c14raw$MaterialType))%in%material_details_after$Material_Details_JPN))
{
  # Identify Mismatches
  material_details_new = unique(data.frame(Material_Classification=c14raw$Material_Details,Material_Details_JPN=c14raw$MaterialType))
  material_details_todo = left_join(x=material_details_new,y=material_details_after,by='Material_Details_JPN')
  # Add Unique Identifier back to master DB to facilitate merge
  c14raw=left_join(c14raw,material_details_after[c('MaterialTypeUniqueID','Material_Details_JPN')],c('MaterialType'='Material_Details_JPN'))
  # Identify new cases and assign UniqueIDs
  index=which(!is.na(material_details_todo$Material_Details_JPN)&is.na(material_details_todo$MaterialTypeUniqueID))
  newIDs = (max(material_details_todo$MaterialTypeUniqueID,na.rm=TRUE)+1):(max(material_details_todo$MaterialTypeUniqueID,na.rm=TRUE)+length(index))
  material_details_todo$MaterialTypeUniqueID[index]=newIDs
  # Add the uniqueIDs also to the main c14raw
  for (i in 1:length(index))
  {
    tmp.match=material_details_todo$Material_Details_JPN[index[i]]
    c14raw$MaterialTypeUniqueID[which(c14raw$MaterialType==tmp.match)]=newIDs[i]
  }
  
  material_details_todo = data.frame(MaterialTypeUniqueID=material_details_todo$MaterialTypeUniqueID,
                                     Material_Classification=material_details_todo$Material_Classification.x,
                                     Material_Details_JPN=material_details_todo$Material_Details_JPN,
                                     inconsistent=material_details_todo$inconsistent,
                                     Materials_Details_EN=material_details_todo$Materials_Details_EN,
                                     Materials_Detail_Identification_EN=material_details_todo$Materials_Detail_Identification_EN,
                                     Materials_Detail_Other_EN=material_details_todo$Materials_Detail_Other_EN,
                                     Materials_Detail_Notes_EN=material_details_todo$Materials_Detail_Notes_EN)
  write.csv(material_details_todo,file=here('lookup','material_details_todo.csv'),row.names=F) #export to lookup table
  
  #NOTE:
  # * Make sure to save the current version of c14raw with the MaterialTypeUniqueID to facilitate merge back / check
}


# If no discrepancies join to main table
tmp.for.join.materials = data.frame(Material_Details_JPN=material_details_after$Material_Details_JPN,
                                    Material_Details2=material_details_after$Materials_Details_EN)
c14raw = left_join(x=c14raw,y=tmp.for.join.materials,by=c("MaterialType"="Material_Details_JPN"))
```
...
```{r}
# For Analyses
write.csv(c14raw,file='japanc14db_v04.0(210715).csv')
c14db=c14raw
save(c14db,file='japanc14db_v04.0(210715).RData')
...
save(c14translation,file='japanc14db_translation_v04.0(210715).RData')
write.csv(c14translation,file='japanc14db_translation_v04.0(210715).csv')
```