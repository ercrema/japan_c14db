...
c14raw=read_xlsx( "./raw/c14_raw_v210218.xlsx", skip=1) %>% as.data.frame(stringsAsFactor=FALSE)
```
...
```{r}
# read translation lookup file, originally created with the following command:
material_details = unique(data.frame(Material_Details_JPN=c14db$MaterialType,Material_Details2=NA))
material_details = arrange(material_details,Material_Details_JPN)
write.csv(material_details,file="./lookup/material_details_toTranslate.csv",row.names=F)
```
...
## Store Output 
```{r}
write.csv(c14raw,file='japanc14db_v03.0(210218).csv')
c14db=c14raw
save(c14db,file='japanc14db_v03.0(210218).RData')
```
